...
class MusicPlayer private constructor() {
    ...
    val stContext = newSingleThreadContext("countUp")
    ...
    fun playMusic(position: Int) { //플레이어에 음원을 세팅하고 재생 (mediaPlayer가 null일때 작동)
        Log.d("현재 재생 곡 위치", "현재 위치 : ${position+1}/${musicList.size}")
        ...
        try {
            ...
            if (state == State.PAUSE) {
                ...
                Log.d(
                    "MusicPlayer",
                    "MusicPosition = ${getCurrentPosition() / 1000000}/${(music.duration ?: 0) / 1000000}"
                )
            }
            ...
            CoroutineScope(stContext).launch {
            ...
            Log.d(
                "MusicPlayer",
                "num = ${musicList.size}, current = ${position + 1}/${musicList.size}"
            )
        } catch(e: Exception) {
            e.printStackTrace()
            Log.d("현재", "error!!!")
            Toast.makeText(mContext, "음원을 재생할 수 없어 플레이어를 종료합니다", Toast.LENGTH_SHORT).show()
            stop()
        }
    }
